# MuuTask (C++ 版) を exe 1 つに固めて、README.txt と一緒に zip にする。
#   powershell -ExecutionPolicy Bypass -File build.ps1
#
# 出来上がるもの:
#   csrc\dist\MuuTask\            そのまま動く配布フォルダー
#     MuuTask.exe                 実行ファイル (CRT は静的リンク・約 0.4 MB)
#     README.txt                  取扱説明 (版番号を埋めたもの)
#     config.json                 設定の初期値
#   csrc\dist\MuuTask-<版>.zip    配布用 (上のフォルダーを固めたもの)
#
# Python 版と違って付属ファイルが無いので、lib\ は要らない。
...
$icon = Join-Path $repo 'muutask.ico'
...
$readme = [IO.File]::ReadAllText((Join-Path $repo 'README.txt'))
...
$emit = Start-Process -FilePath (Join-Path $appdir 'MuuTask.exe') `
